...
study_20_normal_bf <- function(estimate,se,prior_sd = 0.5
  ...
  bf10_two_sided <- exp(
    -0.5 * log1p(prior_ratio) +
      0.5 * z^2 * prior_ratio / (1 + prior_ratio)
  )
  
  posterior_mean <- estimate *
    prior_sd^2 / (prior_sd^2 + se^2)
  ...
  posterior_positive <- stats::pnorm(
    posterior_mean / posterior_sd
  )
  
  bf10_two_sided *
    2 * posterior_positive
}


compute_study_20_bayes_factors <- function(
    claim,
    priors = NULL
) {
  prior_sd <- 0.5
